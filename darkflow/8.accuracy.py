# -*- coding: utf-8 -*-
"""
Created on Wed Mar 21 14:22:13 2018
...
def find_accuracy(path,jpath):
    ...
    for b,file in enumerate(glob.glob("*.xml")):
        ...
        True_Positives.append(len(True_Positive))
        False_Positives.append(len(False_Positive))
        Undetecteds.append(len(undetected))
        accuracies.append(accuracy)
        
    TP = np.sum(np.asarray(True_Positives))
    FP = np.sum(np.asarray(False_Positives))
    FN = np.sum(np.asarray(Undetecteds))
    Precision = TP/(TP+FP)
    Recall = TP/(TP+FN)
...
def main():
    annotation_path="d:/shopee/data/validation/"    
    json_path="d:/shopee/data/validation/out/"    
